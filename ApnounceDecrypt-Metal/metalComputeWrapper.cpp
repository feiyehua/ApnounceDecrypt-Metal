...
void metalComputeWrapper::initWithDevice(MTL::Device* device) {
    ...
    auto functionName = NS::String::string("blockSha1", NS::ASCIIStringEncoding);
...
void metalComputeWrapper::prepareData() {
    ...
    calcBuffer = mDevice->newBuffer(BUFFER_SIZE, MTL::ResourceStorageModeShared);
    result = mDevice->newBuffer(sizeof(uint64_t), MTL::ResourceStorageModeShared);
    mStart = mDevice->newBuffer(sizeof(uint32_t), MTL::ResourceStorageModeShared);
